...
def update_results(framework,model_name,model_size,batchsize,convert_time,load_time):
    ...
    if "onnx" in framework : 
        s3_client.upload_file(f'/tmp/{model_name}_{model_size}_{batchsize}_convert.json',BUCKET_NAME,f'results/convert/tvm/intel/onnx/{model_name}_{model_size}_{batchsize}_convert.json')
        print("upload done : convert time results")
    else:
        s3_client.upload_file(f'/tmp/{model_name}_{model_size}_{batchsize}_convert.json',BUCKET_NAME,f'results/convert/tvm/intel/{model_name}_{model_size}_{batchsize}_convert.json')
        print("upload done : convert time results")
